...
namespace s21
{
  ...
  Vector<value_type>::Vector(size_type n)
  {
    ...
    if (n > max_size())
    {
      throw std::length_error("Невозможно создать s21::Vector больше максимального размера()");
    }
    size_ = n;
    capacity_ = n;
  ...
  // Деструктор
  template <typename value_type>
  Vector<value_type>::~Vector() { remove(); }

  // Операторы присваивания

  // Присваивание копированием
  template <typename value_type>
  typename s21::Vector<value_type> &
  ...
  template <typename value_type>
  typename Vector<value_type>::reference_type
  ...
  // Очистка вектора
  template <typename value_type>
  void Vector<value_type>::clear() { size_ = 0; }
